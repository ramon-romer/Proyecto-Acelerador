...
function exp_h(mixed $value): string
{
    return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
}

function exp_portal_url(): string
{
    return '../aneca_portal/index.php';
}

function exp_index_url(): string
{
    return 'index.php';
}

function exp_listado_url(): string
{
    return 'listado.php';
}

function exp_render_layout_start(string $title, string $subtitle = '', array $breadcrumbs = [], array $actions = []): void
{
    echo '<!DOCTYPE html>';
    echo '<html lang="es">';
    echo '<head>';
    ...
    if ($breadcrumbs !== []) {
        ...
        foreach ($breadcrumbs as $index => $crumb) {
            ...
            $label = exp_h($crumb['label'] ?? '');
            $url = $crumb['url'] ?? null;
            if ($url !== null && $index !== $lastIndex) {
                echo '<a href="' . exp_h((string)$url) . '">' . $label . '</a>';
            } else {
    ...
    echo '<div><h1>' . exp_h($title) . '</h1>';
    if ($subtitle !== '') { echo '<p>' . exp_h($subtitle) . '</p>'; }
    echo '</div>';
    if ($actions !== []) {
        echo '<div class="hero-actions">';
        foreach ($actions as $action) {
            $label = exp_h($action['label'] ?? 'Acción');
            $url = exp_h((string)($action['url'] ?? '#'));
            $class = 'btn ' . exp_h((string)($action['class'] ?? 'light'));
...
function exp_render_layout_end(): void
{
    echo '</main></div></body></html>';
}

function exp_render_result_badge(string $resultado): string
{
    ...
    return '<span class="badge ' . $clase . '">' . exp_h($resultado) . '</span>';
}
