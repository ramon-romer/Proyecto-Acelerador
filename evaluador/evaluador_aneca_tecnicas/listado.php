            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($evaluaciones as $fila): ?>
                        <tr>
                            <td><?= tec_h((string)$fila['id']) ?></td>
                            <td class="nombre"><?= tec_h((string)$fila['nombre_candidato']) ?></td>
                            <td><?= tec_h((string)$fila['area']) ?></td>
                            <td><?= tec_h((string)$fila['categoria']) ?></td>

                            <td class="num"><?= tec_h(tec_nf($fila['bloque_1'] ?? 0)) ?></td>
                            <td class="num"><?= tec_h(tec_nf($fila['bloque_2'] ?? 0)) ?></td>
                            <td class="num"><?= tec_h(tec_nf($fila['bloque_3'] ?? 0)) ?></td>
                            <td class="num"><?= tec_h(tec_nf($fila['bloque_4'] ?? 0)) ?></td>
                            <td class="num"><?= tec_h(tec_nf($fila['total_b1_b2'] ?? 0)) ?></td>
                            <td class="num"><strong><?= tec_h(tec_nf($fila['total_final'] ?? 0)) ?></strong></td>

                            <td><?= tec_render_result_badge((string)$fila['resultado']) ?></td>
                            <td><?= tec_h((string)$fila['fecha_creacion']) ?></td>
                            <td>
