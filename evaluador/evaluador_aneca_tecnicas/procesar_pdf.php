...
$resumen = [
    '1A Publicaciones y patentes' => count($jsonExtraido['bloque_1']['publicaciones']),
    '1B Libros y capítulos' => count($jsonExtraido['bloque_1']['libros']),
    '1C Proyectos y contratos' => count($jsonExtraido['bloque_1']['proyectos']),
...
tec_render_layout_start(
    'Expediente extraído',
    'Revisa el resumen de extracción y elige si quieres evaluar directamente o completar datos manuales antes de recalcular.',
    [
...
<section class="card stack">
    <div class="meta-grid">
        <div class="metric">
            ...
            <span class="value" style="font-size:20px"><?= tec_h($nombreCandidato) ?></span>
        </div>
        <div class="metric">
            <span class="label">Archivo</span>
            <span class="value" style="font-size:18px"><?= tec_h(basename($rutaPdf)) ?></span>
        </div>
        <div class="metric">
    ...
    <div class="resumen-grid">
        <?php foreach ($resumen as $label => $valor): ?>
            <div class="resumen-item">
                <span class="k"><?= tec_h($label) ?></span>
                <span class="v"><?= tec_h((string)$valor) ?></span>
            </div>
        <?php endforeach; ?>
    </div>

    <div class="subinfo">
        Referencia de máximos en Técnicas: B1=60, B2=30, B3=8, B4=2.  
...
<section class="card stack">
    ...
    <div class="toolbar">
        <form action="guardar_evaluacion.php" method="post">
            <input type="hidden" name="nombre_candidato" value="<?= tec_h($nombreCandidato) ?>">
            <textarea name="json_entrada" style="display:none;"><?= tec_h($jsonPlano) ?></textarea>
