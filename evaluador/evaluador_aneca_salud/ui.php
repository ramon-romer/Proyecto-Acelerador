...
function salud_h(mixed $value): string
{
    return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
}

function salud_portal_url(): string
{
    return '../aneca_portal/index.php';
}

function salud_index_url(): string
{
    return 'index.php';
}

function salud_listado_url(): string
{
    return 'listado.php';
}

function salud_render_layout_start(string $title, string $subtitle = '', array $breadcrumbs = [], array $actions = []): void
{
    echo '<!DOCTYPE html>';
    echo '<html lang="es">';
    echo '<head>';
    ...
    echo '<title>' . salud_h($title) . '</title>';
    echo '<style>
        :root {
            --azul-900: #173a77;
            --azul-800: #1e4d9b;
    ...
    if ($breadcrumbs !== []) {
        ...
        foreach ($breadcrumbs as $index => $crumb) {
            ...
            $label = salud_h($crumb['label'] ?? '');
            $url = $crumb['url'] ?? null;
            if ($url !== null && $index !== $lastIndex) {
                echo '<a href="' . salud_h((string)$url) . '">' . $label . '</a>';
            } else {
    ...
    echo '<div><h1>' . salud_h($title) . '</h1>';
    if ($subtitle !== '') { echo '<p>' . salud_h($subtitle) . '</p>'; }
    echo '</div>';
    if ($actions !== []) {
        echo '<div class="hero-actions">';
        foreach ($actions as $action) {
            $label = salud_h($action['label'] ?? 'Acción');
            $url = salud_h((string)($action['url'] ?? '#'));
            $class = 'btn ' . salud_h((string)($action['class'] ?? 'light'));
...
function salud_render_layout_end(): void
{
    echo '</main></div></body></html>';
}

function salud_render_result_badge(string $resultado): string
{
    ...
    return '<span class="badge ' . $clase . '">' . salud_h($resultado) . '</span>';
}
