 ...
 * Helpers
 * ========================================================= */
function normalizar_lista_post(mixed $lista): array
{
    if (!is_array($lista)) {
        return [];
    }
...
function fusionar_listas(array $base, array $nuevos): array
{
    if ($nuevos === []) {
        return $base;
    }
...
/* =========================================================
 * Recoger datos manuales
 * ========================================================= */
$publicaciones = normalizar_lista_post($_POST['publicaciones'] ?? []);
$libros = normalizar_lista_post($_POST['libros'] ?? []);
$proyectos = normalizar_lista_post($_POST['proyectos'] ?? []);
$transferencia = normalizar_lista_post($_POST['transferencia'] ?? []);
$tesisDirigidas = normalizar_lista_post($_POST['tesis_dirigidas'] ?? []);
$congresos = normalizar_lista_post($_POST['congresos'] ?? []);
$otrosInv = normalizar_lista_post($_POST['otros_meritos_investigacion'] ?? []);

$docencia = normalizar_lista_post($_POST['docencia'] ?? []);
$evaluacionDocente = normalizar_lista_post($_POST['evaluacion_docente'] ?? []);
$formacionDocente = normalizar_lista_post($_POST['formacion_docente'] ?? []);
$materialDocente = normalizar_lista_post($_POST['material_docente'] ?? []);

$formacion = normalizar_lista_post($_POST['formacion'] ?? []);
$experiencia = normalizar_lista_post($_POST['experiencia'] ?? []);

$bloque4 = normalizar_lista_post($_POST['bloque4'] ?? []);

/* =========================================================
 * Fusionar con JSON base
 * ========================================================= */
$jsonBase['bloque_1']['publicaciones'] = fusionar_listas(
    $jsonBase['bloque_1']['publicaciones'],
    $publicaciones
);

$jsonBase['bloque_1']['libros'] = fusionar_listas(
    $jsonBase['bloque_1']['libros'],
    $libros
);

$jsonBase['bloque_1']['proyectos'] = fusionar_listas(
    $jsonBase['bloque_1']['proyectos'],
    $proyectos
);

$jsonBase['bloque_1']['transferencia'] = fusionar_listas(
    $jsonBase['bloque_1']['transferencia'],
    $transferencia
);

$jsonBase['bloque_1']['tesis_dirigidas'] = fusionar_listas(
    $jsonBase['bloque_1']['tesis_dirigidas'],
    $tesisDirigidas
);

$jsonBase['bloque_1']['congresos'] = fusionar_listas(
    $jsonBase['bloque_1']['congresos'],
    $congresos
);

$jsonBase['bloque_1']['otros_meritos_investigacion'] = fusionar_listas(
    $jsonBase['bloque_1']['otros_meritos_investigacion'],
    $otrosInv
);

$jsonBase['bloque_2']['docencia_universitaria'] = fusionar_listas(
    $jsonBase['bloque_2']['docencia_universitaria'],
    $docencia
);

$jsonBase['bloque_2']['evaluacion_docente'] = fusionar_listas(
    $jsonBase['bloque_2']['evaluacion_docente'],
    $evaluacionDocente
);

$jsonBase['bloque_2']['formacion_docente'] = fusionar_listas(
    $jsonBase['bloque_2']['formacion_docente'],
    $formacionDocente
);

$jsonBase['bloque_2']['material_docente'] = fusionar_listas(
    $jsonBase['bloque_2']['material_docente'],
    $materialDocente
);

$jsonBase['bloque_3']['formacion_academica'] = fusionar_listas(
    $jsonBase['bloque_3']['formacion_academica'],
    $formacion
);

$jsonBase['bloque_3']['experiencia_profesional'] = fusionar_listas(
    $jsonBase['bloque_3']['experiencia_profesional'],
    $experiencia
);

$jsonBase['bloque_4'] = fusionar_listas(
    $jsonBase['bloque_4'],
    $bloque4
);
