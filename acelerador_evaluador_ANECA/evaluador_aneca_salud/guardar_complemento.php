...
foreach (array_values($_POST['publicaciones'] ?? []) as $item) {
    $publicaciones[] = [
        'tipo' => (string)($item['tipo'] ?? 'articulo'),
        ...
        'numero_autores' => int_post($item['numero_autores'] ?? 5, 5),
        'citas' => int_post($item['citas'] ?? 0, 0),
        'anios_desde_publicacion' => int_post($item['anios_desde_publicacion'] ?? 3, 3),
        'liderazgo' => bool_post($item['liderazgo'] ?? '0'),
    ];
}
...
foreach (array_values($_POST['proyectos'] ?? []) as $item) {
    $proyectos[] = [
        ...
        'tipo_proyecto' => (string)($item['tipo_proyecto'] ?? 'hospitalario'),
        'rol' => (string)($item['rol'] ?? 'investigador'),
        'dedicacion' => (string)($item['dedicacion'] ?? 'parcial'),
        'anios_duracion' => float_post($item['anios_duracion'] ?? 0, 0),
    ];
}

$transferencia = [];
foreach (array_values($_POST['transferencia'] ?? []) as $item) {
    $transferencia[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'tipo' => (string)($item['tipo'] ?? 'protocolo_clinico'),
        'impacto_externo' => bool_post($item['impacto_externo'] ?? '0'),
        'liderazgo' => bool_post($item['liderazgo'] ?? '0'),
        'participacion_menor' => bool_post($item['participacion_menor'] ?? '0'),
    ];
}

$tesis = [];
foreach (array_values($_POST['tesis'] ?? []) as $item) {
    $tesis[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'tipo' => (string)($item['tipo'] ?? 'codireccion'),
        'calidad_especial' => bool_post($item['calidad_especial'] ?? '0'),
    ];
}

$congresos = [];
foreach (array_values($_POST['congresos'] ?? []) as $item) {
    $congresos[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'ambito' => (string)($item['ambito'] ?? 'local'),
        'tipo' => (string)($item['tipo'] ?? 'poster'),
        'id_evento' => (string)($item['id_evento'] ?? uniqid('evento_', true)),
    ];
}

$otrosInvestigacion = [];
foreach (array_values($_POST['otros_investigacion'] ?? []) as $item) {
    $otrosInvestigacion[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'tipo' => (string)($item['tipo'] ?? 'grupo_investigacion'),
    ];
}
...
$evalDocente = [];
foreach (array_values($_POST['eval_docente'] ?? []) as $item) {
    $evalDocente[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'tipo' => (string)($item['tipo'] ?? 'encuestas'),
        'resultado' => (string)($item['resultado'] ?? 'aceptable'),
    ];
}

$formDocente = [];
foreach (array_values($_POST['form_docente'] ?? []) as $item) {
    $formDocente[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'horas' => int_post($item['horas'] ?? 0, 0),
        'rol' => (string)($item['rol'] ?? 'asistente'),
    ];
}

$materialDocente = [];
foreach (array_values($_POST['material_docente'] ?? []) as $item) {
    $materialDocente[] = [
        'es_valido' => bool_post($item['es_valido'] ?? '1'),
        'tipo' => (string)($item['tipo'] ?? 'material_publicado'),
    ];
}
...
$manual = [
    'bloque_1' => [
        ...
        'congresos' => $congresos,
        'otros_meritos_investigacion' => $otrosInvestigacion,
    ],
    'bloque_2' => [
        'docencia_universitaria' => $docencia,
        'evaluacion_docente' => $evalDocente,
        'formacion_docente' => $formDocente,
        'material_docente' => $materialDocente,
    ],
    'bloque_3' => [
